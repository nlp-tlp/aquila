				...
				for(i = 0, len = sCData.length; i < len; i++) {
					...
					for(var g in GRANULARITIES) {
						...
						function getDate(granularity, date) {
							...
							switch(gi) {
								case "Days": 	return "" + date.getDate() + "/" + date.getMonth() + "/" + date.getFullYear(); break;
								case "Weeks": 	var lastSunday = getLastSunday(date);
												return "" + lastSunday.getDate() + "/" + lastSunday.getMonth() + "/" + lastSunday.getFullYear(); break;							
								case "Months": 	return "" + date.getMonth() + "/" + date.getFullYear(); break;
								case "Years": 	return "" + date.getFullYear(); break;
							}
						}
	...
	*/
	...
	function getChartInfo(granularity) {
		chartInfo = function() {
			...
			chartInfo["Days"] = {
				...
				width:  keyLength * 50,
				circleRadius: 14,
				maxY: Math.max(10, d3.max(data, function(d) { return d.eventNumber["Days"]; }))
			}
			...
			chartInfo["Weeks"] = {
				...
				width:  keyLength * 50,
				circleRadius: 8,
				maxY: Math.max(10, d3.max(data, function(d) { return d.eventNumber["Weeks"]; }))
			}
			...
			chartInfo["Months"] = {
				...
				width:  keyLength * 130,
				circleRadius: 4,
				maxY: Math.max(10, d3.max(data, function(d) { return d.eventNumber["Months"]; }))
			}
			...
			chartInfo["Years"] = {
				...
				width:  keyLength * 500,
				circleRadius: 4,
				maxY: Math.max(10, d3.max(data, function(d) { return d.eventNumber["Years"]; }))
			}
	...
	function createChart(granularity) {
		...
		g.selectAll("circle")
		    ...
		    .enter()
		    .append("circle")
		    .attr("class", function(d) { return "colour-" + (ACCIDENT_CATEGORIES.indexOf(d[CATEGORICAL_FIELDNAME]) + 1)})
		    .attr("cx", function (d) { return x(getDateX(d, granularity)); })
		    .attr("cy", 0)
		    .attr("r", chartInfo[granularity].circleRadius)
		    .attr("data-x-Days", 	function(d) 	{ return getXScale(d, "Days") })
		    .attr("data-x-Weeks", 	function(d) 	{ return getXScale(d, "Weeks") })
		    .attr("data-x-Months", 	function(d) 	{ return getXScale(d, "Months") })
		    .attr("data-x-Years", 	function(d) 	{ return getXScale(d, "Years") })
		    .attr("data-y-Days", 	function(d) 	{ return getYScale(d, "Days") })
		    .attr("data-y-Weeks", 	function(d) 	{ return getYScale(d, "Weeks") })
		    .attr("data-y-Months", 	function(d) 	{ return getYScale(d, "Months") })
		    .attr("data-y-Years", 	function(d) 	{ return getYScale(d, "Years") })
		    .attr("data-tip", 		function(d) { 
		    	return "<strong>Date: </strong>" + d[DATE_FIELDNAME] + "<br/>" +
				"<strong>Area: </strong>" + d["Area"] + "<br/>" +
				"<strong>Place of Injury: </strong>" + d["Place Of Injury"] + "<br/>" +
				"<strong>" + CATEGORICAL_FIELDNAME + ": </strong> <span class=\"text colour-" + (ACCIDENT_CATEGORIES.indexOf(d[CATEGORICAL_FIELDNAME]) + 1) + "\">" + d[CATEGORICAL_FIELDNAME] + "</span><br/><hr/>" +
